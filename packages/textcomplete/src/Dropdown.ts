...
import { IEditorCursorOffset, IEventEmitter } from "./interface"
...
export type DropdownEventName = "hidden" | "hide" | "render" | "rendered" | "select" | "selected" | "show" | "shown"
...
class DropdownRenderer {
  ...
  constructor({ className, footer, header, style, item, placement }: IDropdownOptions) {
    this.el = document.createElement("ul")
    this.el.className = className || "textcomplete-dropdown"
  ...
  public setOffset(offset: IEditorCursorOffset): this {
  ...
  private initializeEl(styleOption?: Record<string, string>): void {
    ...
    for (const key of ["header", "footer"]) {
      const li = document.createElement("li")
      li.className = `textcomplete-${key}` // TODO: make configurable
...
export class Dropdown extends EventEmitter implements IEventEmitter<DropdownEventName> {
  public shown: boolean
  public options: IDropdownOptions
  ...
  public init(options: IDropdownOptions) {
    this.options = options
    this.rotate = options.hasOwnProperty("rotate") ? !!options.rotate : this.rotate
  ...
  public render<T>(searchResults: SearchResult<T>[], offset: IEditorCursorOffset): this {
