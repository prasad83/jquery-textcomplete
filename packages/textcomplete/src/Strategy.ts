...
export interface StrategyProps<T = any> {
  ...
  search: (term: string, callback: ISearchCallback<T>, match: RegExpMatchArray) => void
...
export class Strategy<T = any> {
  ...
  public search(term: string, callback: ISearchCallback<T>, match: RegExpMatchArray): void {
    if (this.props.cache) {
  ...
  public createQuery(text: string): IQuery | null {
  ...
  private searchWithCache(term: string, callback: ISearchCallback<T>, match: RegExpMatchArray): void {
    if (this.cache[term]) {
